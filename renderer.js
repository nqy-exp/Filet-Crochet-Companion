...
drawMainView(model) {
    ...
    const OFFSET = 35; // 必须与 resize 中的值一致
    ctx.clearRect(0, 0, canvas.width, canvas.height);

    let rowIndices = [];
    for (let i = 0; i < config.viewRows; i++) {
        let rIdx = model.curR - i;
        if (rIdx >= 0) rowIndices.push(rIdx); else break;
    }

    rowIndices.forEach((rIdx, visualRowIndex) => {
        if (rIdx < 0 || rIdx >= model.totalRows) return;
        ...
        for (let vCol = 0; vCol < config.viewCols; vCol++) {
            ...
            // 【核心】：Y坐标加上 OFFSET，实现整体下移
            const y = OFFSET + (visualRowIndex * config.cellSize);
...
drawUIOverlay(model) {
    ...
    const OFFSET = 35; // 必须与 resize/drawMainView 中的值保持一致

    // --- 1. 行号与箭头 (整体下移) ---
    for (let i = 0; i < config.viewRows; i++) {
        const rIdx = model.curR - i;
        if (rIdx < 0) break;
...
// A. 画【刻度数字】和【点】 (修复了最后一个数字不显示的问题)
for (let cIdx = 0; cIdx <= model.totalCols; cIdx += config.groupSize) { // 使用 <= 确保包含末尾刻度
...
for (let i = 0; i < totalGroups; i++) { // 【关键修复】：这里不再是 totalGroups - 1
    ...
    // 判断中点是否在当前视野内 (增加微小缓冲防止边缘消失)
    if (xMid >= sideMargin - 10 && xMid <= viewWidthLimit) {
        // 按照你的要求：顶部从左往右看是 3, 2, 1
        // 如果 i=0 (第一组 0-5), 它是最右边的，对应编号 1
        // 如果 i=2 (第三组 10-15), 它是最左边的，对应编号 3
        const topGroupNum = totalGroups - i; 

        // 1. 画小组号数字
        ctx.font = `bold ${groupLabelStyle.fontSize}px Arial`;
        ctx.fillStyle = groupLabelStyle.color;
        ctx.fillText(topGroupNum, xMid, topRulerY + (rulerFontSize * 0.3));
...
// A. 画【刻度数字】和【点】
for (let cIdx = 0; cIdx <= model.totalCols; cIdx += config.groupSize) { // 使用 <=
    if (cIdx >= model.curC && cIdx <= model.curC + config.viewCols) {
        const visualCol = cIdx - model.curC;
        const x = sideMargin + (visualCol * config.cellSize);
...
for (let i = 0; i < totalGroups; i++) { // 【关键修复】：这里不再是 totalGroups - 1
    ...
    if (xMid >= sideMargin - 10 && xMid <= viewWidthLimit) {
        ...
        ctx.font = `bold ${groupLabelStyle.fontSize}px Arial`;
        ctx.fillStyle = groupLabelStyle.color;
        ctx.fillText(groupNum, xMid, bottomRulerY + (rulerFontSize * 0.3));
...
drawMinimap(model) {
    ...
    // 【核心逻辑】：获取配置中定义的完成色 (例如 rgba(46, 204, 113, 0.5))
    ...
    for (let r = 0; r < model.totalRows; r++) {
        for (let c = 0; c < model.totalCols; c++) {
            ...
            const miniX = c;
            const miniY = (model.totalRows - 1) - r; 

            if (cell.isDone) {
                // 【核心修复】：处理小地图的透明度显示
                // 我们需要从 rgba(R, G, B, A) 中提取出 RGB，然后手动赋予一个新的、更适合缩略图的 Alpha 值
                const parts = baseCompletionColor.match(/\d+/g);
                if (parts && parts.length >= 3) {
                    // 为了让小地图在各种背景下都清晰，我们使用一个适中的透明度 (例如 0.7)
                    // 这会让它既有“覆盖感”，又不会像主画布那样显得太淡
...
updateMinimapIndicator(model) {
    ...
    // 3. 【核心修复】：计算顶部边缘的百分比位置
    // 我们要找的是：当前视图窗口的最顶端是在哪里。
    // 公式逻辑：(总行数 - 当前所在行 - 视图能看到的行数) / 总行数
    // 使用 Math.max(0, ...) 是为了防止当你钩到图片最上方时，计算结果变成负数导致框飞出顶部
    let topPercent = ((modelTotalRows - model.curR - viewRows) / modelTotalRows) * 100;
    topPercent = Math.max(0, topPercent);

    // 4. 计算左侧边缘的百分比 (保持不变)
    const leftPercent = (model.curC / modelTotalCols) * 100;
...
const InteractionManager = {
    ...
    handleKeyDown(e) {
        const m = this.model;
        // 【新增】：支持 Ctrl + S 或 Cmd + S 进行快速保存
       if ((e.ctrlKey || e.metaKey) && e.key === 's') {
        e.preventDefault(); // 防止浏览器自带的保存网页对话框
        saveProject();
        return;
        }

        switch(e.key) {
            case 'ArrowRight': if (m.curC < m.totalCols - this.renderer.config.viewCols) m.curC++; break;
            case 'ArrowLeft':  if (m.curC > 0) m.curC--; break;
            case 'ArrowUp':    if (m.curR < m.totalRows - 1) m.curR++; break;
            case 'ArrowDown':  if (m.curR > 0) m.curR--; break;
        }
        // 【修复】：HUD 方向同步使用引擎计算
...
handleMouseDown(e) {
    ...
    // 边界检查：如果点在左右边距，或者点在了顶部的标尺区域（ mouseY < OFFSET），直接返回
    ...
    let rIdx = this.model.curR - visualRowIdx;
    ...
    try {
        ...
        if (result && result.success) {
            ...
            if (!loadedData.schema_version) {
                ...
                // [迁移逻辑点]：在这里补齐新版本需要的默认字段
                // 比如以后你加了新功能，就在这里写上：loadedData.config.newFeature = 'default';
            ...
            const progress = loadedData.currentProgress;
            if (progress) {
